...
export function getCookieJSON() {
  const currentCookieJSON = JSON.parse(
    decodeURIComponent(document.cookie.split(";")[0])
  );
  return currentCookieJSON;
}
export function cookieCombine(ObjExpression, age) {
  //cookieにObjExpressionを追加、同じkeyがある場合は上書き / ageは保存日数
...
export function getUserName() {
  //cookieからusernameを取得, requires getCookieValue()
